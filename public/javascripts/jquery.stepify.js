	...
	$.fn.stepify = function(options){
		...
		function bindHandlers(){
			...
			$('.'+opt.nextBtn.cssClass).on('click',function(event){
				...
				if(proceed){
					$sequenceStep.animate({'opacity':0},duration,function(){
		...
		submitBtnStr = '<input type="submit" class="btn btn-success '+opt.submitBtn.cssClass+'" value="'+opt.submitBtn.text+'" ></input>',
		divStr = '<div></div>';
